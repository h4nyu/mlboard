...
impl WorkspaceRepository for Postgresql {
    ...
    fn insert(&mut self, row: &Workspace) -> Result<Uuid, Error> {
        ...
        self.execute(&sql, &[&row.id, &row.name, &row.params, &row.created_at])?;
        Ok(row.id)
    }
    fn update(&mut self, id: &Uuid, name: &str, params: &Value) -> Result<Uuid, Error> {
        let sql = format!(
            "UPDATE {} SET name = $1, params = $2 WHERE id = $3",
            Workspace::table_name()
        );
        self.execute(&sql, &[&name, params, id])?;
        Ok(id.to_owned())
    }
    fn delete(&mut self, id: &Uuid) -> Result<Uuid, Error> {
        let sql = format!("DELETE FROM {} WHERE id = $1", Workspace::table_name());
        self.execute(&sql, &[id])?;
        Ok(id.to_owned())
    }
}

...
impl WorkspaceRepository for Client {
    ...
    async fn update(&self, id: &Uuid, name: &str, params: &Value) -> Result<Uuid, Error> {
        self.execute("UPDATE workspaces SET name = $1, params = $2 WHERE id = $3", &[&name, params, id]).await?;
        Ok(id.to_owned())
    }
    async fn delete(&self, id: &Uuid) -> Result<Uuid, Error> {
        self.execute("DELETE FROM workspaces WHERE id = $1", &[id]).await?;
        Ok(id.to_owned())
    }
}

...
impl PointRepository for Client {
    ...
    async fn bulk_insert(&self, rows:&[&Point]) -> Result<usize, Error> {
        let values_stmt = rows.iter().map(|x| format!("('{}',{},'{}')", x.ts, x.value, x.trace_id)).collect::<Vec<String>>().join(",");
        let sql = format!("INSERT INTO points (ts, value, trace_id) VALUES {}", values_stmt);
        self.execute(&sql[..], &[]).await?;
        Ok(rows.len())
    }

    async fn delete(&self, trace_ids: &[&Uuid]) -> Result<(), Error> {
        self.execute("DELETE FROM points WHERE trace_id = ANY($1)", &[&trace_ids]).await?;
        Ok(())
    }
}
